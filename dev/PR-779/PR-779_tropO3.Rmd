---
title: "PR-779: H2 effects on Trop. O3 RF"
date: "`r Sys.Date()`"
output:
  github_document:
---

Make plots for https://github.com/JGCRI/hector/pull/779 where we add the indirect H2 effects on trop O3 radiative forcing. 
...
(S10) from Dorheim et al. 2024 for our previous $RF_{O_3}$ equation, we had 
...
out2 %>% 
    ...
    labs(title = "H2 Indirect Effects on Trop O3 RF", 
         ...
         x = NULL)
```
So as we would expect we only see changes in the RF_O3 trop and downstream from there. 
...
```{r}
hc <- my_newcore(ini = inifile)

fetchvars(hc, NA, vars = "rho_o3_h2")
````



Questions 
* Where did this coefficient come from? 
